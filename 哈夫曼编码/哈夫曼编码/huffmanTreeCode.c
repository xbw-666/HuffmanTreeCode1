...
void CreateHuff(HuffmanTree HT, DataType* w, int n)
{
	...
	HT = (HuffmanTree*)malloc(m + 1*sizeof(HuffmanTree)); //开m+1个HTNode，因为下标为0的HTNode不存储数据
	int* p =NULL;
...
void HuffCoding(HuffmanTree HT, HuffmanCode HC, int n)
{
	...
	char* code = (char*)malloc(sizeof(char) * n); //编码最长为n(最长时，前n-1个用于存储数据，最后1个用于存放'\0')
